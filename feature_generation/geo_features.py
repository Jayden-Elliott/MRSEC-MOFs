...
def get_features(folder):
    if not os.path.exists(folder + "zeo/"):
        os.mkdir(folder + "zeo/")
    zeo_folder = folder + "packages/zeo/"
    ...
    for filename in os.listdir(folder + "cifs/"):
        ...
        try:
            if not os.path.exists(folder + "primitives/" + filename):
                get_primitive(folder + "cifs/" + filename, folder + "primitives/" + filename)
            name = filename[:-4]

            cmd1 = PATHNAME + "zeo++-0.3/network -ha -res " + zeo_folder + name + "_pd.txt " + cif_folder + filename
            cmd2 = PATHNAME + "zeo++-0.3/network -sa 1.86 1.86 10000 " + zeo_folder + name + "_sa.txt " + cif_folder + filename
            cmd3 = PATHNAME + "zeo++-0.3/network -volpo 1.86 1.86 10000 " + zeo_folder + name + "_pov.txt " + cif_folder + filename
    ...
    df.to_csv(folder + "geo_featurization.csv", index=False)
